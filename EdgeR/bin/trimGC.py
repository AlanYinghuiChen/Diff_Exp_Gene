...
output_file_1 = args.output_file+'_{}.fq.gz'.format(matemap["1"])
output_file_2 = args.output_file+'_{}.fq.gz'.format(matemap["2"])
input_file_1 = args.input_file+'_{}.fq.gz'.format(matemap["1"])
input_file_2 = args.input_file+'_{}.fq.gz'.format(matemap["2"])
...
for label_1,label_2 in zip(input_1,input_2):
    ...
    sequence_1 = next(input_1).strip()  #the 2nd line
...
#    match_2 = re.search(patterns[1],sequence_2.decode())
    match_1 = re.search("^[GC]{3,}",sequence_1.decode())
    match_2 = re.search("[GC]{3,}$",sequence_2.decode())
    length_1 = 0 if match_1 is None else len(match_1.group(0))
    length_2 = 0 if match_2 is None else len(match_2.group(0))
#   if trimmed read is shorten than minimum length, discard it.
    if len(sequence_1)-length_1 < min_length or len(sequence_2)-length_2 < min_length:
        too_short += 1
        _ = next(input_1)  #the 3th line
        _ = next(input_2)
        _ = next(input_1)  #the 4th line
        _ = next(input_2)
        continue
#   trim the TSO sequence on the reads
...
#   write the trimmed reads
    ...
    output_1.write(next(input_1))   #the 3th line
    output_2.write(next(input_2))
#   trim quality sequence of the TSO  the 4th line
...
#   collect trimmed reads' length data
