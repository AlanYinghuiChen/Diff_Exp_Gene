...
log2FC_cutoff = float(sys.argv[1])
FDR_cutoff = float(sys.argv[2])
diffExp_file=sys.argv[3]
Exp_file=sys.argv[4]
geneName=sys.argv[5]
outFile_all = sys.argv[6]
outFile_filtered_up = sys.argv[7]
outFile_filtered_down = sys.argv[8]
mapStat_file = sys.argv[9]
...
for line in DIFF_EXP:
	...
	if gene in geneID_name_map:
		...
		if logFC == "NA" or FDR == "NA":
			continue
		elif float(logFC) > log2FC_cutoff and float(FDR) <= FDR_cutoff:
